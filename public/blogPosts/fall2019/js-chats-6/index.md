...
- [Introduction to Testing with JavaScript](#introduction-to-testing-with-javascript)
  - [JavaScript Chats with ACM Hack Session 6](#javascript-chats-with-acm-hack-session-6)
...
on Wikipedia](/blogPosts/fall2019/js-chats-6/images/cowboy-warning.png)
...
favorite is called **Test Driven Development** (TDD).
...
different types of tests](/blogPosts/fall2019/js-chats-6/images/test-pyramid.png)
...
describe('/fizzBuzz endpoint', () => {
	it('should respond to request', done => {
		request(app).get('/fizzBuzz/1').expect(200).end(done);
	});
});
```
...
it('should not respond to request without number', async () => {
	await request(app).get('/fizzBuzz/').expect(404);
});
```
...
We use a common testing technique called **mocking**.
